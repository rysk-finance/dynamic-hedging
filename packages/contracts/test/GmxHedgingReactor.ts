...
describe("GMX Hedging Reactor", () => {
	...
	it("deploys mock chainlink price feed and hooks to GMX Vault Price feed", async () => {
		...
		const mockChainlinkFeed = await mockChainlinkFeedFactory.deploy()
	...
	it("opens a long position", async () => {
		...
		const gmxReader = await ethers.getContractAt("IReader", gmxReaderAddress)
		...
		// const currentPriceAfter = parseFloat(
		// 	utils.formatEther(await priceFeed.getNormalizedRate(wethAddress, usdcAddress))
		// )

		// console.log({ currentPriceBefore, currentPriceAfter })
