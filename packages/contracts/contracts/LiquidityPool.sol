...
contract LiquidityPool is ERC20, Ownable, AccessControl, ReentrancyGuard, Pausable {
  ...
  */
		...
		returns (uint256 quote, int256 delta)
	{
		...
		int256 newDelta = PRBMathSD59x18.abs(portfolioDelta - quoteState.totalDelta);
		...
		uint256 normalizedDelta = uint256(newDelta < 0 ? -newDelta : newDelta).div(_getNAV());
		...
		if (quoteState.isDecreased) {
			...
			uint256 discount = quoteState.deltaTiltFactor > maxDiscount
				? maxDiscount
				: quoteState.deltaTiltFactor;

			// discounted BS option price
			uint256 newOptionPrice = quoteState.totalOptionPrice - discount.mul(quoteState.totalOptionPrice);
			// discounted utilization priced option
			quoteState.utilizationPrice =
				quoteState.utilizationPrice -
				discount.mul(quoteState.utilizationPrice);
			// quote the greater of discounted utilization or discounted BS
			quote = quoteState.utilizationPrice > newOptionPrice
				? quoteState.utilizationPrice
				: newOptionPrice;
		} else {
			uint256 newOptionPrice = quoteState.deltaTiltFactor.mul(quoteState.totalOptionPrice) +
			...
			if (quoteState.utilizationPrice < maxPrice) {
				...
				quoteState.utilizationPrice =
					quoteState.deltaTiltFactor.mul(quoteState.utilizationPrice) +
					quoteState.utilizationPrice;
				quote = quoteState.utilizationPrice > newOptionPrice
					? quoteState.utilizationPrice
					: newOptionPrice;
