 ...
 */
contract PortfolioValuesFeed is AccessControl {
	...
	// the number of options for a specific series
	mapping(bytes32 => OptionStores) public hashForSeries;
	// the series for a specific issuance hash

	// series to loop over stored as issuance hashes
	bytes32[] public seriesToLoop;
	// portfolio values
	...
	function fulfill(
		...
		uint256 callPutsValue;
		uint length = seriesToLoop.length;
		// get the spot price
		uint256 spotPrice = PriceFeed(priceFeed).getNormalizedRate(_underlying, _strikeAsset);
		uint256 rfr = 0;
		for (uint i=0; i < length; i++) {
			// get series
			OptionStores memory _optionStores = hashForSeries[seriesToLoop[i]];
			// get the vol
	...
	function updateStores(
		...
		_isHandler();
		// make a hash for the series and store it somewhere
		bytes32 issuanceHash = getIssuanceHash(_optionSeries);
		// if the hash doesnt exist in the map then put it in the array and increment it
		if (hashForSeries[issuanceHash].amount == 0) {
			// maybe store them by expiry instead
			seriesToLoop.push(issuanceHash);
			hashForSeries[issuanceHash] = OptionStores(
				_optionSeries,
				_amount
			);
		} else {
			// increment the amount for series
			hashForSeries[issuanceHash].amount += _amount;
		}

		emit StoresUpdated(_optionSeries, _amount, issuanceHash);
	}
