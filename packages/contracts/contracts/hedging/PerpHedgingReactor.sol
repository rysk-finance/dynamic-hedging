...
contract PerpHedgingReactor is IHedgingReactor, AccessControl {
	...
	function update() public returns (uint256) {
		...
		(, , IClearingHouse.CollateralDepositView[] memory collatDeposits, ) = clearingHouse_
		...
		uint256 currentPrice = OptionsCompute.convertToDecimals(
			PriceFeed(priceFeed).getNormalizedRate(wETH, collateralAsset_),
    ...
    */
	function _changePosition(int256 _amount) internal returns (int256) {
		...
		if (collatToDeposit > 0) {
			...
			clearingHouse_.swapToken(accountId_, poolId, swapParams);
			try clearingHouse_.updateMargin(accountId_, collateralId, -int256(collatToWithdraw)) {} catch (
				bytes memory reason
			) {
