...
contract NewController is Initializable, OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe {
	...
	mapping(address => mapping(uint256 => MarginVault.VaultLiquidationDetails))
		internal vaultLiquidationDetails;
	...
	function isSettlementAllowed(address _otoken) external view returns (bool) {
		(address underlying, address strike, address collateral, uint256 expiry) = _getOtokenDetails(
			_otoken
		);
		...
		onlyAuthorized(msg.sender, _args.owner)
	{
		...
		(MarginVault.Vault memory vault, uint256 typeVault, ) = getVaultWithDetails(
			_args.owner,
			_args.vaultId
		);
	...
	function _redeem(Actions.RedeemArgs memory _args) internal {
		...
		(address collateral, address underlying, address strike, uint256 expiry) = _getOtokenDetails(
			address(otoken)
		);
	...
	function _liquidate(Actions.LiquidateArgs memory _args) internal notPartiallyPaused {
		...
		) = _isLiquidatable(_args.owner, _args.vaultId);
		...
		MarginVault.VaultLiquidationDetails storage vaultLiqDetails = vaultLiquidationDetails[
			_args.owner
		][_args.vaultId];
		...
		nakedPoolBalance[vault.collateralAssets[0]] = nakedPoolBalance[vault.collateralAssets[0]].sub(
			collateralToSell
		);
