...
contract NewMarginCalculator is Ownable {
	...
	function getNakedMarginRequired(
		...
		bytes32 productHash = _getProductHash(_underlying, _strike, _collateral, _isPut);
		ShortScaledDetails memory ssd = makeShortScaledDetails(
			_shortAmount,
			_strikePrice,
			_underlyingPrice
		);
		...
		returns (FPI.FixedPointInt memory)
	{
		...
		for (uint8 i = 0; i < timesToExpiry.length; i++) {
			if (timesToExpiry[i] >= optionTimeToExpiry)
				return
					FPI.fromScaledUint(maxPriceAtTimeToExpiry[_productHash][timesToExpiry[i]], SCALING_FACTOR);
		}
	}
	...
	function _getCallSpreadMarginRequired(
		 ...
		 *                             long strike
		 */
		FPI.FixedPointInt memory firstPart = _longStrike.sub(_shortStrike).mul(_shortAmount).div(
			_longStrike
		);
		...
		returns (VaultDetails memory)
	{
		...
		if (vaultDetails.hasLong) {
			...
			vaultDetails.longCollateralDecimals = uint256(
				ERC20Interface(vaultDetails.longCollateralAsset).decimals()
			);
		}
	 ...
	 */
	function _checkIsValidVault(MarginVault.Vault memory _vault, VaultDetails memory _vaultDetails)
		internal
		view
	{
	...
	function _getCashValue(
		...
		if (_isPut)
			return _strikePrice.isGreaterThan(_underlyingPrice) ? _strikePrice.sub(_underlyingPrice) : ZERO;
