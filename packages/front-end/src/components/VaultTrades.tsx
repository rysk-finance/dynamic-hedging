...
const parseTrades = (data: any) => {
  ...
  const allTrades = [...writeOptionsActions, ...buybackOptionActions, ...rebalanceDeltaActions].map( trade => {

    const tradeType = ["WriteOptionsAction","BuybackOptionAction"].includes(trade.__typename) ? "optionsTrade" : "deltaRebalance";
          ...
          <a  href={`${SCAN_URL[chainId]}/tx/${trade.transactionHash}`} 
            ...
            <div className="grid grid-cols-12 py-2 text-black hover:bg-black hover:text-white px-2">
              ...
              <div className="col-span-2">
                ...
                { trade.typename === "BuybackOptionAction" && "Buyback Options" }
                { trade.typename === "RebalanceDeltaAction" && "Delta Rebalance" }
              </div>
              <div className="col-span-3">
                { trade.optionSymbol }
              </div>
              ...
              <div className="col-span-1 text-right">
                { trade.premium && trade.typename === "WriteOptionsAction" && 
                  <NumberFormat
                    value={(
              ...
              <div className="col-span-1 text-right">
                { trade.premium && trade.typename === "BuybackOptionAction" &&
                  <NumberFormat
                    value={(
                      Number(trade.premium) /
                      10 ** DECIMALS.RYSK
